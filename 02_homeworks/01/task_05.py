...
if company_profit > 0:
    ...
    print(
        f'The breakeven is {int(round((company_profit/company_income) * 100, 0))} percent')
    employee_count = int(
        input('Please enter total quantity of the company employees: '))
    print(
        f'The profit per employee = {round(company_profit / employee_count, 2)}')
else:
    print(
        f'Your company has no profit, you lost {company_income - company_costs}')
